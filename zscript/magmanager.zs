...
class MagManager:HDWeapon{
	...
	override void DropOneAmmo(int amt){
		...
		if(!what)return;
		int howmany=min(max(1,getdefaultbytype((class<hdpickup>)(mmm.roundtype)).bulk?HD_MAXPOCKETSPACE/getdefaultbytype((class<hdpickup>)(mmm.roundtype)).bulk*0.2:1),owner.countinv(what),100);
		if(!howmany)return;
